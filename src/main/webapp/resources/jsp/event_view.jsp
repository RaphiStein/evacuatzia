    ...
    <div class="row">
      ...
      <div class="col-lg-6">
        <div class="row">
          ...
          <div class="theData col-lg-8"><p> ${event.location} </p></div>
        </div>
        <div class="row">
          <c:set var="eventDate" value="${event.estimatedTime}" />
          <div class="theLabel col-lg-4"><p> Estimated Time: </p></div>
          <div class="theData col-lg-8"><p><fmt:formatDate type="both" dateStyle="short" timeStyle="short" value="${eventDate}" /></p></div>
        </div>
        <div class="row">
          <div class="theLabel col-lg-4"><p> Means of Evacuation: </p></div>
          <div class="theData col-lg-8"><p> ${event.meansOfEvacuation} </p></div>
        </div>
        <div class="row">
          <div class="theLabel col-lg-4"><p> Capacity: </p></div>
          <div class="theData col-lg-8"><p> ${event.capacity} </p></div>
        </div>
        <div class="row">
          <div class="theLabel col-lg-4"><p> Participants: </p></div>
          <div class="theData col-lg-8"><p> ${event.registrationCount} </p></div>
        </div>
        <c:choose>
          <c:when test="${isAdmin}">
            <div class="row">
          ...
          <c:otherwise>
            <c:choose>
              <c:when test="${isRegisteredToEvent}">
                ...
                <div class="row"> 
                  <button type="button" class="btn btn-default">Leave this Event</button>
                </div>
              </c:when>
              <c:otherwise>
                <div class="row"> 
